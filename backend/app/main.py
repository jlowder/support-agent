...
class _TraceBroadcaster:
    ...
    def broadcast(
        self, component: str, message: str, payload: Optional[dict] = None
    ) -> None:
...
@tool(
    description="Look up customer profile by customer ID (e.g., usr_001) or email address. Returns customer details including loyalty tier, order count, and order history.",
...
def get_customer_profile(customer_id: str) -> str:
    ...
    profile = {
        ...
        "recent_orders": customer.get("order_history", [])[
            :3
        ],  # Last 3 orders with full item details
...
@tool(
    description="Get raw order items for an order. Returns item details including name, price, quantity, is_opened status, and item_type. No eligibility determination - just raw data for the LLM to evaluate.",
    response_format="content",
)
...
@tool(
    description="Escalate an issue to a human customer service representative. Use when the LLM cannot make a decision, customer requests supervisor, or refund amount exceeds $500."
)
...
@tool(
    description="Select specific items from an order for refund. Takes order_id and item description/pattern to find matching items. Use this when customer specifies which items they want to return."
)
def select_items_for_refund(order_id: str, item_selection: str) -> str:
    ...
    # Handle numeric selection (1-indexed)
        ...
        try:
            ...
            for item in items:
                ...
                if (
                    item_selection_lower in item_name
                    or item_name in item_selection_lower
                ):
    ...
    response["total_refund"] = sum(
        item["refunded_amount"] for item in response["matched_items"]
    )
...
async def chat(request: ChatRequest):
    ...
    try:
        ...
        traceback.print_exc()
        trace_broadcaster.broadcast(
            "error", f"Agent loop error: {str(e)}", {"error": str(e)}
        )
        ...
        process_refund.run(
            {
                ...
                "reason": "Refund requested via test",
            }
        )
    )
