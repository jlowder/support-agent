            ...
            if order.get("order_id") == order_id:
                ...
                if clause == "time_window":
                    ...
                    if days_since_purchase > eligible_days:
                        result["valid"] = False
                        result["reasons"].append(f"Order is {days_since_purchase} days old, exceeds {eligible_days}-day limit for {loyalty_tier} tier")
                
                elif clause == "condition":
                    has_opened_items = any(item.get("opened", False) for item in items)
                    if has_opened_items:
                        result["reasons"].append("Some items in order have been opened (15% restocking fee applies)")
                    ...
                    if non_refundable_items:
                        result["valid"] = False
                        result["reasons"].append(f"Order contains non-refundable items: {[item['name'] for item in non_refundable_items]}")
                
                elif clause == "defective":
                    defective_items = [
                        item for item in items 
                        if item.get("defective", False)
                    ]
                    if defective_items:
                        result["reasons"].append(f"Defective items detected: {[item['name'] for item in defective_items]} - 15% fee waived")
...
def process_refund_transaction_fn(order_id: str, amount: float) -> str:
    ...
    if _RETRY_COUNTS[key] == 0 and int(amount * 100) % 2 == 1:
        _RETRY_COUNTS[key] += 1
        raise Exception("503 Service Unavailable: Connection reset by peer during payment gateway transaction")
...
def build_agent():
    ...
    def authenticate_customer(state: AgentState) -> AgentState:
        ...
        if customer_id:
            ...
            if "error" not in user_data:
                ...
                state["context"]["loyalty_tier"] = user_data.get("loyalty_tier", "Standard")
    ...
    def extract_refund_request(state: AgentState) -> AgentState:
        ...
        if is_refund_request:
            ...
            order_matches = re.findall(r'(ORD-\d{4}-\d{2}-\d{2}-\d+)', full_content.upper())
            if order_matches:
    ...
    def evaluate_policy(state: AgentState) -> AgentState:
        ...
        if reasons:
            ...
            state["context"]["action"] = "deny_refund_policy_violation"
            state["context"]["deny_reason"] = "Refund request outside eligible time window"
        else:
    ...
    def generate_response(state: AgentState) -> AgentState:
        ...
        if action == "refund_success":
            ...
            response += f"A refund of ${state.get('refund_amount'):.2f} has been issued. "
            response += f"Transaction ID: {state['context'].get('transaction_id')}. "
            response += "You should see the credit on your statement within 3-5 business days."
            ...
            response = "I need a bit more information to help you with your refund request. "
            ...
            response += f"Error: {state['context'].get('failure_reason', 'Unknown error')}. "
    ...
    workflow.add_conditional_edges(
        "init",
        lambda s: "authenticate" if not s["context"].get("customer_authenticated") else "extract",
        ["authenticate", "extract"]
    )
    
    workflow.add_conditional_edges(
        "authenticate",
        lambda s: "extract" if not s["context"].get("current_order_id") else "check_policy",
        ["extract", "check_policy"]
    )
    
    workflow.add_conditional_edges(
        "extract",
        lambda s: "check_policy" if s["context"].get("current_order_id") else "generate_response",
    ...
    workflow.add_conditional_edges(
        "evaluate",
        lambda s: "process" if s["context"].get("action") == "process_refund" else "generate_response",
...
async def chat_endpoint(request: ChatRequest):
    try:
        ...
        if request.customer_id:
            messages.append(HumanMessage(content=f"My customer ID is {request.customer_id}"))
        elif request.email:
        ...
        result = await agent.ainvoke(
            {"messages": messages, "context": {}, "current_order_id": None, "refund_amount": None},
            config=config
        )
        
        return {
            "response": result["messages"][-1].content if result["messages"] else "I couldn't process your request.",
            "messages": [m.dict() for m in result["messages"]]
        }
    
    except Exception as e:
        await broadcast_trace(TraceEvent(
...
async def voice_ingress_endpoint():
    ...
    return {
        ...
        "next_steps": ["Integrate OpenAI Whisper for STT", "Integrate ElevenLabs or OpenAI TTS for Egress"]
    }
