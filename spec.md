...
| --- | --- | --- | --- | --- |
| **SCENARIO-01** | Happy Path | User orders clothing item 10 days ago, item is unopened, requests refund with order ID. | Agent calls `get_user_profile`, verifies date is under 30 days, references policy, executes `process_refund_transaction`. | Shows clear tool invocation chain and successful transaction ID generation. |
| **SCENARIO-02** | Holding the Line | User has a Bronze profile, bought an item 50 days ago, insists on a refund. | Agent calls `get_user_profile`, identifies date violates the 30-day limit, politely denies refund, offers store credit alternative. | Displays policy checking step returning false with violation reasons, followed by the text-generation output. |
| **SCENARIO-03** | Error Recovery | User triggers a valid refund path that targets the transient failure mock function. | Agent encounters database exception on first execution attempt, logs error, re-invokes tool successfully. | Highlighted warning/exception block on screen, followed immediately by a successful execution block. |
| **SCENARIO-04** | Authentication Flow | User sends message without customer ID or email. | Agent detects missing authentication, generates response asking for customer ID or email. | Shows `request_auth_info` node execution with payload indicating `action: request_authentication`. |
| **SCENARIO-05** | Order Selection Flow | User provides customer ID but not order ID. | Agent lists user's orders with order ID, total, and refund status, asks user to select. | Shows `list_orders` node execution with order history payload, followed by user message selecting an order. |
| **SCENARIO-06** | Digital Item Rejection | User requests refund for a digital download item (type: "digital"). | Agent calls `check_policy`, LLM identifies digital items as non-refundable per policy, denies refund. | Shows policy check with violation reasons for digital items, response denies refund with explanation. |
...
* [x] FastAPI backend with LangGraph state machine
* [x] Authentication via customer ID or email extraction
* [x] Order history listing and order selection
* [x] Policy validation using LLM with structured JSON output
* [x] Refund transaction processing with simulated API timeout
* [x] Natural language response generation
* [x] SSE-based admin trace logging
* [x] Retry mechanism for transient failures
* [x] Escalation to human agent tool

### 5.2 Pending / Future Enhancements

* [ ] Voice pipeline integration (STT/TTS)
